...
function Extension() {
  ...
  const originalCartAmount = useMemo(() => {
    return cartLines.reduce((sum, line) => {
      const productId = line?.merchandise?.product?.id;

      if (!productId) return sum;
      if (productId === EGIFT_PRODUCT_ID) return sum;
      if (giftProductIds.includes(productId)) return sum;

      return sum + Number(line?.cost?.totalAmount?.amount || 0);
    }, 0);
  }, [cartLines, giftProductIds]);

  const finalCartAmount = useMemo(() => {
    const giftAmount = cartLines.reduce((sum, line) => {
      const productId = line?.merchandise?.product?.id;

      if (
        productId === EGIFT_PRODUCT_ID ||
    ...
    return Math.max(Number(total?.amount || 0) - giftAmount, 0);
  }, [cartLines, total, giftProductIds]);

  const discountRatio = useMemo(() => {
    if (originalCartAmount <= 0) return 1;

    return Math.min(finalCartAmount / originalCartAmount, 1);
  }, [finalCartAmount, originalCartAmount]);
  ...
  const collectionAmountInfos = useMemo(() => {
    return collectionAmountConditions.map((condition) => {
      ...
      const discountedCollectionAmount =
        originalCollectionAmount * discountRatio;

      const currentAmount = condition.collectionOnly
        ? discountedCollectionAmount
        : finalCartAmount;
      ...
      return {
        ...
        title:
          ...
          condition.collection?.title ||
          "対象コレクション",

        collectionOnly: condition.collectionOnly,
        collectionTitle: condition.collection?.title,
  ...
  const activeProgressInfo = useMemo(() => {
    ...
    const matchedConditions = relatedConditions
      .filter((info) => info.isConditionMatched)
      .sort((a, b) => {
    ...
    return [...relatedConditions]
      .map((info) => {
        const amountProgress =
          info.thresholdAmount > 0
            ? Math.min(info.currentAmount / info.thresholdAmount, 1)
            : 0;

        const quantityProgress =
          info.requiredQuantity > 0
            ? Math.min(
                info.collectionQuantity / info.requiredQuantity,
                1
              )
            : 0;
      ...
      .sort((a, b) => {
        if (b.conditionProgress !== a.conditionProgress) {
          return b.conditionProgress - a.conditionProgress;
        }
      ...
      return (
        <s-stack direction="block" gap="small-200">
          ...
          {!info.isAmountMatched ? (
            ...
            <s-text>
              対象商品をあと {info.remainingQuantity}点追加してください。
            </s-text>
          ) : (
            <s-text>
              プレゼントの獲得条件を達成しました。
